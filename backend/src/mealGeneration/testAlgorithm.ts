...
async function testMealAlgorithm() {
  ...
  try {
    ...
    const plan1 = await mealAlgorithm(
      3, // 3 days
      400, // 400 cal per meal (flexible range for all meal times)
    ...
    plan1.forEach((day, index) => {
      ...
      console.log(`  Lunch: ${day.lunch.name} (${day.lunch.calories} cal)`);
      console.log(`  Dinner: ${day.dinner.name} (${day.dinner.calories} cal)`);
    });
